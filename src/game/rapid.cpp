...
const float heatValue = 5.f;
const float damageAmount = 10.f;
const float fireRate = 0.04f;


Rapid::Rapid()
	: m_bShooting(false)
{
	m_fCooldownSpeed = 45.f;
}
...
bool 
Rapid::Initialise(Renderer& renderer, FMOD::System& fmodSystem)
{
	m_pSprite = renderer.CreateSprite("assets\\sprites\\weapon\\rapid.png", SpriteOrigin::BOTTOM_CENTER);

	m_pSprite->SetScale(1.4f);
	m_pShotSprite = renderer.CreateSprite("assets\\sprites\\weapon\\shot.png", SpriteOrigin::CENTER);
	m_pShotSprite->SetBlueTint(0.7f);
	m_pShotSprite->SetRedTint(0.6f);
	m_pShotSprite->SetScaleY(0.6f);

	m_pIconSprite = renderer.CreateSprite("assets\\sprites\\ui\\rapid-icon.png", SpriteOrigin::CENTER);
	m_pIconSprite->SetScale(3.f);

	m_pFMODsystem = &fmodSystem;
	fmodSystem.createSound("assets\\sounds\\rapid.wav", FMOD_DEFAULT, NULL, &m_pShotSound);

	return true;
}

void
Rapid::Process(float deltaTime, Input& input)
{
	static XInputController* controller = input.GetController(0);
	Weapon::Process(deltaTime, input);

	if (m_bShooting&& controller->GetButtonState(SDL_CONTROLLER_BUTTON_RIGHTSHOULDER) == BS_RELEASED)
	{
		m_bShooting = false;
	}

	static float timeUntilNextFire = fireRate;
	if (m_bShooting)
	{
		timeUntilNextFire -= deltaTime;
		if (timeUntilNextFire <= 0.f)
		{
			Shoot(m_autoShotPosition, m_fAutoShotAngle);
			timeUntilNextFire = fireRate;
		}
	}
	

	for (unsigned int shotIndex = 0; shotIndex < Rapid::sm_uiLoadedShots; ++shotIndex)
	{
...
void 
Rapid::Draw(Renderer& renderer)
{
	for (unsigned int i = 0; i < Rapid::sm_uiLoadedShots; ++i)
	{
		if (m_shots[i].active)
		{
...
void 
Rapid::Shoot(Vector2 position, float angle)
{
...
void
Rapid::SetAutoAttributes(Vector2 position, float angle)
{
	m_autoShotPosition = position;
	m_fAutoShotAngle = angle;
}